...
function loadAllFormsToView(){
	...
	for(each of requestedForms){		
		...
		text_details.innerText = each.small_description;
		
		index.setAttribute("index",each.form_id);
		index.setAttribute("link",each.link);
	...
	if(requestedForms.length <= 0){
		_("form_list_main").innerHTML = "No matching Item Found for this name... :-( ";
	}
...
function getQueryForms(){
	...
	let search_input = _("search_input").value ? _("search_input").value : undefined;
		...
		if (search_input) {
			let collected = decople(toolsCollections);
...
function openForm(elm){
	...
	// if(inIframe()){
		// postMessageToParent('openModal:{"link":"'+indexTarget+'","custom_class":"no_close_button,blurred"}');
	// }else{
		// open_modal(indexTarget,'no_close_button,blurred');
	// }
